...
describe('Order Entity', () => {
    ...
    const mockAddress = new Address_1.Address({
        street: '123 Main St',
        city: 'New York',
        state: 'NY',
        country: 'USA',
    ...
    describe('Order Creation', () => {
        test('should create an order with valid data', () => {
            const order = Order_1.Order.create('user-123', mockItems, mockAddress);
            expect(order.userId).toBe('user-123');
            expect(order.status).toBe(Order_1.OrderStatus.PENDING);
            expect(order.totalAmount).toBe(999.99);
            expect(order.items).toHaveLength(1);
            expect(order.shippingAddress).toBe(mockAddress);
            expect(order.orderId).toBeInstanceOf(OrderId_1.OrderId);
        ...
        test('should calculate total amount correctly with multiple items', () => {
            ...
            const order = Order_1.Order.create('user-123', multipleItems, mockAddress);
            expect(order.totalAmount).toBe(2029.97); // (999.99 * 2) + 29.99
        });
        test('should throw error when userId is empty', () => {
            expect(() => {
                Order_1.Order.create('', mockItems, mockAddress);
            }).toThrow('User ID is required');
        });
        test('should throw error when items array is empty', () => {
            expect(() => {
                Order_1.Order.create('user-123', [], mockAddress);
    ...
    describe('Order Status Updates', () => {
        test('should confirm order when status is PENDING', () => {
            ...
            order.confirm();
            expect(order.status).toBe(Order_1.OrderStatus.CONFIRMED);
        ...
        test('should ship order when status is CONFIRMED', () => {
            ...
            order.confirm();
            order.ship();
            expect(order.status).toBe(Order_1.OrderStatus.SHIPPED);
        });
        ...
        test('should cancel order when status is not DELIVERED', () => {
            const order = Order_1.Order.create('user-123', mockItems, mockAddress);
            order.cancel();
            expect(order.status).toBe(Order_1.OrderStatus.CANCELLED);
        });
    ...
    describe('Order Serialization', () => {
        ...
        test('should reconstruct order from plain object', () => {
            const order = Order_1.Order.create('user-123', mockItems, mockAddress);
            const orderObject = order.toObject();
            const reconstructedOrder = Order_1.Order.fromObject(orderObject);
    ...
    describe('Order Getters', () => {
        test('should return immutable items array', () => {
            ...
            items.push(new OrderItem_1.OrderItem({
                productId: 'prod-999',
                productName: 'Fake',
                quantity: 1,
                unitPrice: 1,
